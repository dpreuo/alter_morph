...
def hubb_hamiltonian(
    ...
    ham = np.zeros((2*lattice.n_vertices, 2*lattice.n_vertices), dtype=ham_type)
    ...
    for n_edge in range(lattice.n_edges):
        ...
        ham[2 * e0 : 2 * e0 + 2, 2 * e1 : 2 * e1 + 2] = h0_term
    ham += ham.T.conj()
    ...
    for n in range(lattice.n_vertices):
        ...
        ham[2 * n : 2 * n + 2, 2 * n : 2 * n + 2] += (
            np.diag(U * total_magnetisation *np.array([-1, 1])
                     + U * total_density)
        )
